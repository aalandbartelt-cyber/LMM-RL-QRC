...
- [ ] **Step 1: Add a shell runner and launcher validation tests**
...
def run_bash(script: str, *, args=(), env=None):
    merged_env = os.environ.copy()
    if env:
        merged_env.update(env)
    return subprocess.run(
        ["bash", "-c", script, "bash", *args], cwd=REPOSITORY_ROOT,
        env=merged_env, text=True, capture_output=True, check=False,
    )
```
...
def test_gpu_selection_defaults_to_two_physical_gpus(self):
    result = run_bash(
        'QRC_SOURCE_ONLY=1 source rl_control/legacy_isaacgym/'
...
def test_gpu_selection_accepts_one_gpu(self):
    result = run_bash(
        'QRC_SOURCE_ONLY=1 QRC_EVAL_GPUS=1 source '
        'rl_control/legacy_isaacgym/run_formal_evaluation_matrix.sh; '
        'parse_gpu_selection; printf "%s\\n" "${PHYSICAL_GPUS[*]}"'
    )
    self.assertEqual(result.returncode, 0, result.stderr)
...
def test_gpu_selection_rejects_invalid_values(self):
    for value in ("", "0 0", "gpu0", "-1", "0 1 2"):
        result = run_bash(
            'export QRC_SOURCE_ONLY=1; export QRC_EVAL_GPUS="$1"; '
...
The `args` tuple passes invalid values, including the empty string, without
shell interpolation.

- [ ] **Step 2: Add failing one-GPU queue and manifest tests**
...
- [ ] **Step 3: Add failing dynamic worker/coordinator and status tests**
...
- [ ] **Step 1: Parse and validate the selected GPU list**
...
After `nvidia-smi -L`, validate every selected index against the reported physical count. Do not silently drop a failing GPU or fall back to another topology.

- [ ] **Step 2: Persist topology and parameterize smoke/queues**
...
- [ ] **Step 3: Parameterize worker lifecycle and coordinator acceptance**
...
- [ ] **Step 4: Add definition-only source mode**
...
- [ ] **Step 1: Read selected topology with legacy fallback**
...
- [ ] **Step 2: Report dynamic PIDs and markers**
...
```bash
python -m unittest discover -s rl_control/tests -p "test_*.py" -v
```
